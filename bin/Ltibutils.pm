...
@EXPORT = qw( parse_dotconfig gm_yyyymmdd parse_config
             ...
             src_tree_is_clean tag_repository export_repository get_scm_tags 
...
sub get_file
{
    ...
    foreach my $dir ( @sdirs ) {
        ...
        if(-f $path) {
            ...
            $refmd5 = get_ref_md5($file, $cf, @sdirs);
            return md5sum_ok($path, $refmd5) ? $path : undef;
        }
    }

    # try to get the reference md5sum for this file
    $refmd5 = get_ref_md5($file, $cf, @sdirs) unless $refmd5;
...
sub test_pp_list
{
    ...
    print("No network download connection available\n") unless $ret;
    print "\n";
    return $ret;
}
...
TXT
    ...
    my $pad_opt  = '-p';
    $pad_opt = '--pad=' . ($fs_size * 1024) if $pcf->{DEPLOYMENT_PADDING_KB};
...
then
    ...
    then
	    ...
	    ln -sf $tdir/rootfs.ext2.gz.uboot $tdir/rootfs_image
            ...
            ln -sf $tdir/rootfs.ext2.gz $tdir/rootfs_image
    fi
fi
if [ "$pcf->{DEPLOYMENT_JFFS2}" = "y" ]
then
...
if [ "$pcf->{DEPLOYMENT_UBIFS}" = "y" ]
then
    echo "createing ubifs image: $tdir/rootfs.ubifs.img";
...
then
    ...
    ln -sf $tdir/tmp/romfs.img $tdir/rootfs_image
...
sub check_scm_id_is_remote
{
    ...
    my ($id, $br) = @_;
    return if `git cherry origin/$br $id` =~ m,^\+,m;
    return 1;
}
...
sub tag_repository
{
    my ($tag) = @_;
    if(-d "$cf->{top}/.git") {
        system_nb("git tag -m $tag $tag") == 0 or return;
    }
